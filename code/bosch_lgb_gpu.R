...
if (interactive()) {
  ...
  threads <- commandArgs(trailingOnly = TRUE)[2]
...
spec_log <- paste0("../lgb_", type, "/", threads, "t_bosch_", model_type)
...
temp_model <- lgb.train(params = list(learning_rate = 0.02,
                                      max_depth = 4,
                                      num_leaves = 15,
                                      max_bin = 255,
                                      min_gain_to_split = 1,
...
for (i in 1:nrow(grid_search)) {
  ...
  StartTime <- System$currentTimeMillis()
  temp_model <- lgb.train(params = list(learning_rate = grid_search$Eta[i],
                                        max_depth = grid_search$Depth[i],
                                        num_leaves = grid_search$Leaves[i],
                                        max_bin = 255,
                                        min_gain_to_split = 1,
